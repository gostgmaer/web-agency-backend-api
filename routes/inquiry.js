...
router.post("/", createInquiryValidation, validateRequest, sanitizeInput, async (req, res, next) => {
	try {
		const inquiryData = {
			...
			subject: req.body.subject || req.body.message?.subject,
			projectType: req.body.projectType || req.body.requirements?.[0],
			...
			newsletterOptIn: req.body.newsletterOptIn ?? req.body.preferences?.newsletterOptIn ?? false,
			privacyConsent: req.body.privacyConsent ?? req.body.preferences?.privacyConsent ?? false,
			source: 'website',
			referrer: req.body.referrer || req.get('Referer') || undefined,
			ipAddress: req.ip,
